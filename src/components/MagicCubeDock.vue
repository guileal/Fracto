...
<template>
  <aside
    ...
    :class="{
      ...
      'logo-dock--materials-collapsed': !editorExpanded,
      'logo-dock--dragged': panelOpen && dockPos !== null,
      'logo-dock--dragging': isDragging,
    }"
    :style="dockStyle"
    aria-label="Editor do cubo mágico"
  >
    <div
    ...
    >
      <p class="logo-dock__title">Editor do cubo</p>
      <button
        v-if="panelOpen"
        type="button"
        class="logo-dock__toggle"
        :aria-expanded="editorExpanded"
        aria-controls="magic-cube-dock-body"
        @click="toggleEditor"
      >
        {{ editorExpanded ? 'Minimizar' : 'Parâmetros' }}
      </button>
      <span v-else class="logo-dock__peek-hint" aria-hidden="true">Abrir</span>
    </div>

    <div v-show="panelOpen" id="magic-cube-dock-body" class="logo-dock__body">
      ...
      <div
        ...
        id="magic-cube-dock-params"
        class="logo-dock__materials"
      >
        <p class="logo-dock__section">Material cubos</p>
        ...
        <p class="logo-dock__section">Timing da animação</p>

        <label class="logo-dock__row">
          <span class="logo-dock__label">Giro fatia</span>
          <input
            type="range"
            :min="MAGIC_CUBE_TIMING.sliceDuration.min"
            :max="MAGIC_CUBE_TIMING.sliceDuration.max"
            :step="MAGIC_CUBE_TIMING.sliceDuration.step"
            :value="config.sliceDuration"
            @input="onNumberInput('sliceDuration', $event)"
          />
          <output class="logo-dock__value">{{ config.sliceDuration.toFixed(2) }}s</output>
        </label>

        <label class="logo-dock__row">
          <span class="logo-dock__label">Explosão</span>
          <input
            type="range"
            :min="MAGIC_CUBE_TIMING.explodeDuration.min"
            :max="MAGIC_CUBE_TIMING.explodeDuration.max"
            :step="MAGIC_CUBE_TIMING.explodeDuration.step"
            :value="config.explodeDuration"
            @input="onNumberInput('explodeDuration', $event)"
          />
          <output class="logo-dock__value">{{ config.explodeDuration.toFixed(2) }}s</output>
        </label>

        <label class="logo-dock__row">
          <span class="logo-dock__label">Reset</span>
          <input
            type="range"
            :min="MAGIC_CUBE_TIMING.resetDuration.min"
            :max="MAGIC_CUBE_TIMING.resetDuration.max"
            :step="MAGIC_CUBE_TIMING.resetDuration.step"
            :value="config.resetDuration"
            @input="onNumberInput('resetDuration', $event)"
          />
          <output class="logo-dock__value">{{ config.resetDuration.toFixed(2) }}s</output>
        </label>

        <label class="logo-dock__row">
          <span class="logo-dock__label">Pausa início</span>
          <input
            type="range"
            :min="MAGIC_CUBE_TIMING.waitStart.min"
            :max="MAGIC_CUBE_TIMING.waitStart.max"
            :step="MAGIC_CUBE_TIMING.waitStart.step"
            :value="config.waitStart"
            @input="onNumberInput('waitStart', $event)"
          />
          <output class="logo-dock__value">{{ config.waitStart.toFixed(1) }}s</output>
        </label>

        <label class="logo-dock__row">
          <span class="logo-dock__label">Pré-explosão</span>
          <input
            type="range"
            :min="MAGIC_CUBE_TIMING.waitBeforeExplode.min"
            :max="MAGIC_CUBE_TIMING.waitBeforeExplode.max"
            :step="MAGIC_CUBE_TIMING.waitBeforeExplode.step"
            :value="config.waitBeforeExplode"
            @input="onNumberInput('waitBeforeExplode', $event)"
          />
          <output class="logo-dock__value">{{ config.waitBeforeExplode.toFixed(1) }}s</output>
        </label>
      </div>

      <div class="logo-dock__export">
        <p class="logo-dock__export-hint">
          Iluminação fixa igual a /logo-fracto-light. Posicione com «Alinhamento e tamanho»,
          ajuste materiais aqui e copie para fixar os defaults antes do build WP.
        </p>
        <button type="button" class="logo-dock__copy" @click="copyDefaultsReference">
          {{ copyFeedback || 'Copiar configurações aprovadas' }}
        </button>
      </div>
    </div>
  </aside>
