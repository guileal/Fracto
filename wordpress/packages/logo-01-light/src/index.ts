...
import { DEFAULT_FRACTO_LOGO_LIGHT_CONFIG } from '@fracto/lib/fractoLogoConfig'
import { FractoLogoScene } from '@fracto/three/FractoLogoScene'

export const ASSET_ID = 'logo-01-light' as const

const LOGO_SELECTOR = `[data-fracto-3d="${ASSET_ID}"]`
const LOGO_CLASS = 'fracto-3d-logo-01-light'
...
export function mount(container: HTMLElement): FractoLogoHandle {
  ...
  const scene = new FractoLogoScene(ensureCanvas(container), DEFAULT_FRACTO_LOGO_LIGHT_CONFIG)
...
export interface FractoLogoApi {
  ...
  defaults: typeof DEFAULT_FRACTO_LOGO_LIGHT_CONFIG
}
...
declare global {
  interface Window {
    Fracto3d?: {
      ...
      logo01Light?: FractoLogoApi
    }
    FractoLogo01Light?: FractoLogoApi
  }
}
...
const api: FractoLogoApi = {
  ...
  unmount,
  autoInit,
  defaults: DEFAULT_FRACTO_LOGO_LIGHT_CONFIG,
}
...
if (typeof window !== 'undefined') {
  ...
  window.Fracto3d.logo01Light = api
  window.FractoLogo01Light = api
